...
describe("note mentions", () => {
  ...
  afterAll(async () => {
    if (createdNoteIds.length > 0) {
      await db
        ...
        .where(inArray(notifications.noteId, createdNoteIds));
      await db
        .delete(roundNoteMentions)
        .where(inArray(roundNoteMentions.noteId, createdNoteIds));
  ...
  it("fans out one notification, suppresses inaccessible users, and does not notify on edit without new mentions", async () => {
    ...
    const preview = await notesService.previewMentions(actor, round.id, [
      lead.id,
      outsider.id,
    ]);
    expect(
      preview.mentions.find((row) => row.userId === lead.id)?.canRead
    ).toBe(true);
    expect(
      preview.mentions.find((row) => row.userId === outsider.id)?.canRead
    ).toBe(false);
    ...
    const leadNotes = await db
      ...
      .where(
        and(
          eq(notifications.userId, lead.id),
          eq(notifications.noteId, created.id)
        )
      );
    ...
    const outsiderNotes = await db
      ...
      .where(
        and(
          eq(notifications.userId, outsider.id),
          eq(notifications.noteId, created.id)
        )
      );
  ...
  it("renders the mentioned user's current name from the token id, not a stored display string", async () => {
    ...
    await db
      .update(users)
      .set({ name: "William Stocks" })
      .where(eq(users.id, lead.id));

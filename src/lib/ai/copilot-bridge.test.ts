...
const rawBody = JSON.stringify({ tool: "search_notes", input: { query: "roof" } });

function signed(overrides: Partial<Parameters<typeof signCopilotToolRequest>[0]> = {}) {
  const input = {
    principalId: "42",
    tool: "search_notes",
    timestamp: Date.now(),
...
describe("copilot tool bridge", () => {
  ...
  it("verifies a matching HMAC and rejects a mismatch", () => {
    ...
    expect(verifyCopilotToolRequest({ ...input, hmac: "deadbeef" })).toBe(false);
    expect(verifyCopilotToolRequest({ ...input, principalId: "99", hmac })).toBe(false);
  ...
  it("rejects a signature when the body was tampered with", () => {
    ...
    expect(
      verifyCopilotToolRequest({
        ...input,
        rawBody: JSON.stringify({ tool: "search_notes", input: { query: "salaries" } }),
        hmac,
      }),
    ).toBe(false);
  });
  ...
  it("rejects timestamps outside the allowed skew window (replay protection)", () => {
    ...
    expect(verifyCopilotToolRequest({ ...stale.input, hmac: stale.hmac, now })).toBe(false);

    const future = signed({ timestamp: now + COPILOT_TOOL_MAX_SKEW_MS + 1 });
    expect(verifyCopilotToolRequest({ ...future.input, hmac: future.hmac, now })).toBe(false);

    const fresh = signed({ timestamp: now - COPILOT_TOOL_MAX_SKEW_MS + 1000 });
    expect(verifyCopilotToolRequest({ ...fresh.input, hmac: fresh.hmac, now })).toBe(true);

    const invalid = signed({ timestamp: Number.NaN });
    expect(verifyCopilotToolRequest({ ...invalid.input, hmac: invalid.hmac, now })).toBe(false);
  });
  ...
  it("matches the signature produced by the agent-side app bridge", () => {
    ...
    const secret = process.env.BETTER_AUTH_SECRET?.trim() || "precon-demo-copilot";
  ...
  describe("hosted deployments without a configured secret", () => {
    ...
    it("fails closed instead of accepting the well-known dev fallback", () => {
      ...
      expect(verifyCopilotToolRequest({ ...input, hmac: "forged-with-public-fallback" })).toBe(
        false,
      );

...
import { GET as databricksGet, POST as databricksPost } from "@/app/api/jobs/databricks-sync/route";
...
describe("external sync jobs", () => {
  it("rejects unauthenticated cron salesforce sync", async () => {
    const res = await salesforcePost(new Request("http://localhost/api/jobs/salesforce-sync", { method: "POST" }));
  ...
  it("runs incremental salesforce sync idempotently without row previews", async () => {
    ...
    const first = await salesforceSyncService.runIncremental(principal, { pageSize: 50 });
    expect(first).not.toHaveProperty("candidates");
    expect(first.opportunitiesSeen).toBeGreaterThanOrEqual(0);
    const second = await salesforceSyncService.runIncremental(principal, { pageSize: 50 });

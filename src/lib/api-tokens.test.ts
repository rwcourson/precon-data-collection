...
describe("api tokens", () => {
  ...
  it("requires a future expiry within the configured maximum", () => {
    ...
    expect(
      validateTokenExpiry(new Date("2026-09-01T12:00:00.000Z"), 30, now).ok
    ).toBe(true);
    expect(
      validateTokenExpiry(new Date("2026-08-09T11:59:59.000Z"), 30, now).ok
    ).toBe(false);
    expect(
      validateTokenExpiry(new Date("2027-08-09T12:00:00.000Z"), 30, now).ok
    ).toBe(false);
  });

...
describe("post-bid region tab and finalize seam", () => {
  ...
  afterAll(async () => {
    if (createdValueIds.length > 0) {
      await db.delete(customColumnValues).where(inArray(customColumnValues.id, createdValueIds));
    }
  });
  ...
  it("Central Heavy Civil rounds get the Central tab; Georgia rounds do not", async () => {
    ...
    expect(centralTab?.columns.some((c) => c.key === "spoilDisposalSite")).toBe(true);
  ...
  it("saves and reloads a region custom column via custom_column_values", async () => {
    ...
    const [job] = await db
      .select({ id: jobs.id, region: jobs.region, preconDepartment: jobs.preconDepartment })
      .from(jobs)
    ...
    const [round] = await db
      ...
      .returning();
    const actor = createPrincipal({ user: rpd, authSource: "sso", workspaceRegion: "Central" });
    ...
    const [row] = await db
      ...
      .where(and(eq(customColumnValues.columnId, col.id), eq(customColumnValues.roundId, round.id)));
    expect(row?.value).toBe("Landfill A (demo)");
    if (row) {
      createdValueIds.push(row.id);
      await db.delete(customColumnValues).where(eq(customColumnValues.id, row.id));
    }
  ...
  it("lock gate ignores region custom columns", async () => {
    const [row] = await db
      ...
      .limit(1);
    const missing = missingRequiredFields(row.round, {}, {
      jobNumber: row.job.jobNumber,
      jobName: row.job.jobName,
      estimateLeadName: "Lead",
    });
    expect(missing.join(" ")).not.toMatch(/demo|River Mile|Spoil Disposal|Clean Room/i);
    const gate = evaluateLockGate(row.round, {}, {
      jobNumber: row.job.jobNumber,
      jobName: row.job.jobName,
      estimateLeadName: "Lead",
    });
    if (!gate.ok) {
      expect(gate.missingFields.join(" ")).not.toMatch(/demo|River Mile|Spoil Disposal|Clean Room/i);
    }
  });

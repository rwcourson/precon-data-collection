...
describe("effort notes", () => {
  ...
  afterAll(async () => {
    if (createdNoteIds.length > 0) {
      await db
        .delete(roundNoteAttachments)
        .where(inArray(roundNoteAttachments.noteId, createdNoteIds));
      await db
        .delete(auditLog)
        .where(
          and(
            eq(auditLog.entity, "round_note"),
            inArray(auditLog.entityId, createdNoteIds)
          )
        );
  ...
  it("creates, edits, and soft-deletes with author timestamps and edited marker", async () => {
    ...
    const edited = await notesService.edit(
      actor,
      created.id,
      "Updated drawing due date — confirmed."
    );
  ...
  it("denies list and create when the principal cannot read the round", async () => {
    ...
    await expect(
      notesService.list(actor, centralRound.id)
    ).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
    await expect(
      notesService.create(actor, centralRound.id, "should not land")
    ).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
  });
  ...
  it("rejects oversized attachments, accepts any type, and downloads with attachment disposition", async () => {
    ...
    Object.defineProperty(oversized, "byteLength", {
      value: NOTE_ATTACHMENT_MAX_BYTES + 1,
    });
    await expect(
    ...
    const binary = await notesService.create(
      actor,
      round.id,
      "any type is fine",
      [
    ...
    const response = await downloadAttachment(
      new NextRequest(
        `http://localhost/api/notes/attachments/${attachment.id}`
      ),
    ...
    form.set(
      "files",
      new File([Buffer.from(PNG)], "api-plan.png", { type: "image/png" })
    );
  ...
  it("accepts a 10,000-character note and rejects 10,001", async () => {
    ...
    await expect(
      notesService.create(actor, round.id, "x".repeat(10_001))
    ).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
  });
  ...
  it("lists a 50-note thread without dropping rows", async () => {
    ...
    const listed = await notesService.list(

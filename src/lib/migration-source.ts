...
export function sourceYears(files: string[]): number[] {
  ...
  for (const file of files) {
    ...
    for (const match of base.matchAll(/(?<!\d)(20[1-4]\d)(?!\d)/g)) years.add(Number(match[1]));
  }
...
export function describeSheet(file: string): { region: string; sheet: string } {
  ...
  const region =
    { "00": "Corporate", CAR: "Carolinas", CEN: "Central", FL: "Florida", GA: "Georgia", TX: "Texas" }[
      prefix
    ] ?? prefix;
  return { region, sheet };
}
...
export function skipReason(file: string): string {
  if (/Self_Perform/i.test(file)) return "Self-perform scope — would double-count rounds";
  if (/Checklist/i.test(file)) return "Process checklist, not estimate data";
  if (/Consolidated/i.test(file)) return "Pre-aggregated rollup of sheets already imported";
  if (/Backup|Dashboard/i.test(file)) return "Dashboard backing data, derived from imported sheets";
  if (/Roster/i.test(file)) return "Staff roster";
  if (/Scoreboard/i.test(file)) return "Derived scoreboard";
  if (/Cost_Tracking/i.test(file)) return "Monthly precon cost tracking — outside the BRD scope";

...
function resource(
  partial: Partial<ResourceDescriptor> = {}
): ResourceDescriptor {
  return {
    type: "job",
    id: 10,
...
describe("authorization principal contract", () => {
  it.each(ROLES)(
    "carries explicit identity, workspace, Region and token state for %s",
    (role) => {
      const value = principal(
        role,
        ROLES.indexOf(role) + 1,
        role === "corporate_admin" ? null : "Central"
      );
      expect(value.authSource).toBe("sso");
      expect(value.user.role).toBe(role);
      expect(value.workspace.kind).toBe(
        role === "corporate_admin" ? "corporate" : "region"
      );
  ...
  it("intersects API-token Region and scope constraints with the user's scope", () => {
    ...
    expect(
      authorize(value, "read", resource({ region: "Florida" })).allowed
    ).toBe(false);
  });
...
describe("deny-by-default role, Region, ownership, publication, field and ACL matrix", () => {
  it.each(ROLES)(
    "applies same/cross Region read and edit policy for %s",
    (role) => {
      ...
      expect(
        authorize(actor, "read", resource({ region: "Florida" })).allowed
  ...
  it("allows leadership cross-Region reads only from an explicit corporate workspace", () => {
    ...
    expect(
      authorize(leadership, "read", resource({ region: "Florida" })).allowed
    ).toBe(true);
  });
  ...
  it("covers every capability and denies unsupported role/resource combinations", () => {
    ...
    for (const capability of capabilities) {
      const target = resource({
        type:
          capability === "restore" || capability === "permanent-delete"
            ? "trash"
            : "job",
        deleted: capability === "restore" || capability === "permanent-delete",
      });
    ...
    expect(
      authorize(principal("rpd"), "distribute", resource({ type: "report" }))
        .allowed
    ).toBe(true);
    expect(
      authorize(
    ...
    expect(
      authorize(
        principal("rpd"),
        "restore",
        resource({ type: "trash", deleted: true })
      ).allowed
    ).toBe(true);
    expect(
      authorize(
    ...
    expect(
      authorize(
        principal("corporate_admin", 9, null),
        "permanent-delete",
        resource({ type: "trash", deleted: true })
      ).allowed
    ).toBe(true);
...
describe("Jay-roadmap capabilities — allow and deny per role", () => {
  ...
  it.each(ROLES)(
    "notes.write / notes.attach allow every in-region role including %s",
    (role) => {
      const actor = principal(
        role,
        ROLES.indexOf(role) + 1,
      ...
      const round = resource({
        type: "round",
        round: { status: "upcoming", region: "Central" },
      });
    ...
    expect(
      authorize(
        ...
        resource({
          type: "round",
          region: "Florida",
          round: { status: "upcoming", region: "Florida" },
        })
      ).allowed
    ).toBe(false);
  });
  ...
  it.each(ROLES)("visibility.manage-region own-region for %s", (role) => {
    ...
    expect(
      authorize(
        actor,
        "visibility.manage-region",
        resource({ type: "job", region: "Central" })
      ).allowed
    ...
    expect(
      authorize(
        actor,
        "visibility.manage-region",
        resource({ type: "job", region: "Florida" })
      ).allowed
    ).toBe(role === "corporate_admin");
  });

  it.each(ROLES)(
    "visibility.assign-user is corporate_admin-only for %s",
    (role) => {
      const actor = principal(
        role,
        ROLES.indexOf(role) + 1,
      ...
      expect(
        authorize(actor, "visibility.assign-user", resource({ type: "job" }))
          .allowed
      ).toBe(role === "corporate_admin");
    }
  );
  ...
  it.each(ROLES)("reports.schedule for %s", (role) => {
    ...
    expect(
      authorize(
        actor,
        "reports.schedule",
        resource({ type: "report", ownerId: actor.user.id, published: true })
      ).allowed
    ).toBe(true);
  });
  ...
  it("denies reports.schedule on another owner's report", () => {
    ...
    expect(
      authorize(
        actor,
        "reports.schedule",
        resource({ type: "report", ownerId: 99, published: true })
      ).allowed
    ).toBe(false);
  });
  ...
  it("denies in-place edit of a standard dashboard", () => {
    ...
    expect(
      authorize(admin, "edit", { ...standard, ownerId: admin.user.id }).allowed
    ).toBe(false);
  });
});
...
describe("database-scoped authorization loaders", () => {
  ...
  beforeAll(async () => {
    ...
    const [createdSheet] = await db
      ...
      .values({
        kind: "grid",
        name: "Authorization matrix sheet",
        region: "Central",
        folder: "Tests",
        ownerId: pcm.id,
      })
    ...
    const [createdReport] = await db
      .insert(savedReports)
      .values({
        name: "Authorization shared report",
        ownerId: pcm.id,
        config: {
          fields: [],
          filters: [],
    ...
    const [createdJob] = await db
      .insert(jobs)
      .values({
        jobNumber: `AUTH-${Date.now()}`,
        jobName: "Deleted authorization job",
        region: "Central",
        preconDepartment: "Central",
        createdById: pcm.id,
        deletedAt: new Date(),
      })
  ...
  it("applies Region and soft-delete predicates before returning a predictable ID", async () => {
    ...
    const [cross] = await db
      ...
      .limit(1);
    expect(
      same && (await loadJobForPrincipal(central, same.id))?.value.id
    ).toBe(same?.id);
  ...
  it("applies persisted ACL, ownership/publication, sharing, admin, and trash state", async () => {
    ...
    const corpPrincipal = createPrincipal({
      user: corp,
      authSource: "sso",
      workspaceRegion: null,
    });
    expect(
      await loadSheetForPrincipal(rpdPrincipal, sheetId, "manage")
    ).not.toBeNull();
    expect(
      await loadDashboardForPrincipal(rpdPrincipal, dashboardId)
    ).toBeNull();
    expect(
      await loadDashboardForPrincipal(pcmPrincipal, dashboardId)
    ).not.toBeNull();
    expect(await loadReportForPrincipal(rpdPrincipal, reportId)).not.toBeNull();
    expect(
      await loadAdminSectionForPrincipal(corpPrincipal, "tokens", "manage")
    ).not.toBeNull();
    expect(
      await loadTrashForPrincipal(
        corpPrincipal,
        "job",
        deletedJobId,
        "permanent-delete"
  ...
  it("loads rounds through scoped SQL and enforces persisted field overrides", async () => {
    ...
    const [round] = await db
      ...
      .where(
        and(
          eq(estimateRounds.region, "Central"),
    ...
    try {
      ...
      await db
        .delete(fieldWritePolicies)
        .where(eq(fieldWritePolicies.id, policy.id));
    }
  });
});

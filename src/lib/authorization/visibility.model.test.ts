...
describe("job visibility model", () => {
  ...
  afterAll(async () => {
    for (const pin of extraPinKeys) {
      await db
        ...
        .where(
          and(
            eq(jobUserVisibility.jobId, pin.jobId),
            eq(jobUserVisibility.userId, pin.userId)
          )
        );
    }
    for (const row of extraRegionKeys) {
      await db
        ...
        .where(
          and(
            eq(jobRegionVisibility.jobId, row.jobId),
            eq(jobRegionVisibility.region, row.region)
          )
        );
    }
    ...
    if (createdJobIds.length > 0) {
      await db
        .delete(jobRegionVisibility)
        .where(inArray(jobRegionVisibility.jobId, createdJobIds));
  ...
  it("backfill: every job has ≥1 region row and the home region row exists", async () => {
    const jobRows = await db
      .select({ id: jobs.id, region: jobs.region })
      .from(jobs);
    const vis = await db
      .select({
    ...
    const missingHome = jobRows.filter(
      (job) =>
        !vis.some((row) => row.jobId === job.id && row.region === job.region)
    );
  ...
  it("a job with region rows for Central and Georgia appears for both principals", async () => {
    ...
    const centralPcm = allUsers.find(
      (row) => row.role === "pcm" && row.region === "Central"
    )!;
    ...
    const added = await visibilityService.addRegion(
      georgiaActor,
      centralJob.id,
      "Georgia"
    );
    ...
    expect(
      (await loadJobForPrincipal(georgiaActor, centralJob.id))?.value.id
    ).toBe(centralJob.id);
    expect(
      (await loadJobForPrincipal(centralActor, centralJob.id))?.value.id
    ).toBe(centralJob.id);
    ...
    const [round] = await db
      ...
      .where(
        and(
          eq(estimateRounds.jobId, centralJob.id),
    ...
    if (round) {
      expect(
        (await loadRoundForPrincipal(georgiaActor, round.id))?.value.round.id
      ).toBe(round.id);
    }
    ...
    const audits = await db
      ...
      .where(
        and(
          eq(auditLog.entityId, centralJob.id),
          eq(auditLog.action, "visibility.add_region")
        )
      );
  ...
  it("a job_user_visibility pin makes the job visible outside the user's region", async () => {
    ...
    const pinned = await visibilityService.addUser(
      admin,
      centralJob.id,
      floridaPcm.id
    );
    ...
    await expect(
      visibilityService.addUser(pcmCannotPin, centralJob.id, floridaPcm.id)
  ...
  it("directors cannot add another region's visibility row", async () => {
    ...
    await expect(
      visibilityService.addRegion(actor, centralJob.id, "Florida")
    ).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
    await expect(
      visibilityService.removeRegion(actor, centralJob.id, "Central")
    ).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
  });

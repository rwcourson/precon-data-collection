...
export async function softDeleteRound(
  ...
  const [updated] = await db
    ...
    .where(
      and(eq(estimateRounds.id, roundId), isNull(estimateRounds.deletedAt))
    )
...
export async function softDeleteSheet(
  principal: Principal,
  sheetId: number
): Promise<number> {
  assertTrashManager(principal);
...
export async function restoreEntity(
  ...
  await withTransaction(async (tx) => {
    if (entityType === "job") {
      ...
      if (batchId != null) {
        await tx
          ...
          .where(
            and(
              eq(estimateRounds.jobId, entityId),
              eq(estimateRounds.deletionBatchId, batchId)
            )
          );
      }
      return;
    ...
    if (entityType === "sheet") {
      ...
      if (!sheet?.deletedAt)
        throw DomainError.badRequest("Sheet is not in trash");
      ...
      if (batchId != null) {
        await tx
          ...
          .where(
            and(
              eq(sheetRows.sheetId, entityId),
              eq(sheetRows.deletionBatchId, batchId)
            )
          );
      }
      return;
...
export async function listTrash(principal: Principal): Promise<TrashItem[]> {
  ...
  const parentSheetIds = [
    ...new Set(gridRows.filter((r) => r.deletedAt).map((r) => r.sheetId)),
  ];
...
export async function permanentDelete(input: {
  ...
  if (input.confirmation !== "PERMANENTLY DELETE") {
    throw DomainError.badRequest(
      'Typed confirmation must be exactly "PERMANENTLY DELETE".'
    );
  }
  ...
  if (input.entityType === "sheet_row") {
    if (input.principal.user.role !== "corporate_admin") {
      throw DomainError.forbidden(
        "Only corporate admins can permanently delete."
      );
    }
    const [row] = await db
      .select()
      .from(sheetRows)
      .where(
        and(eq(sheetRows.id, input.entityId), isNotNull(sheetRows.deletedAt))
      );
    if (!row) throw DomainError.notFound("Deleted sheet row not found");
    const sheet = await loadSheetForPrincipal(
      input.principal,
      row.sheetId,
      "manage"
    );
  ...
  const runMutation = async (tx: typeof db) => {
    if (input.entityType === "round") {
      const [round] = await tx
        ...
        .where(
          and(
            eq(estimateRounds.id, input.entityId),
            isNotNull(estimateRounds.deletedAt)
          )
        );
      if (!round) throw DomainError.notFound("Deleted round not found");
      await tx
        .delete(roundMultiValues)
        .where(eq(roundMultiValues.roundId, round.id));
      await tx
...
export async function materializeBackupForRestore(
  periodKey: string,
  targetDir: string
) {
  const payload = await readSnapshotPayload(periodKey);

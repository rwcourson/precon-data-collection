    ...
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription>
            Restore to bring records back, or permanently delete (manager approval required).
          </CardDescription>
        </CardHeader>
        <CardContent className="px-0 pb-0">
          {items.length === 0 ? (
            <p className="px-6 pb-6 text-sm text-muted-foreground">Trash is empty.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                ...
                {items.map((item) => (
                  <TableRow key={`${item.entityType}-${item.entityId}`}>
                    ...
                    <TableCell className="pr-4">
                      <TrashActions entityType={item.entityType} entityId={item.entityId} />
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>

...
import { Database, Loader2, Play, Eye } from "lucide-react";
    ...
    startTransition(async () => {
      try {
        ...
        if (res.status === "failed") {
          ...
          toast.success(`Wrote ${res.rows.toLocaleString()} rows to ${res.table}`);
        } else {
          toast.success(
            `Built ${res.rows.toLocaleString()} rows${configured ? " (preview only)" : " — credentials not configured"}`,
          );
        }
        if (res.preview) setPreview(JSON.stringify(res.preview[0] ?? {}, null, 2));
        router.refresh();
      } catch (e) {
    ...
    <Card>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1">
            ...
            <CardDescription>
              ...
              calculated metric set, and each Region&apos;s custom columns as a JSON
              map so a new column never needs a warehouse schema change. Point a
              scheduler at{" "}
      ...
      <CardContent className="space-y-3">
        <div className="grid gap-2 text-xs sm:grid-cols-3">
          ...
          <Fact
            label="Last run"
            value={lastRunLabel ? `${lastRunLabel} · ${state.lastStatus}` : "Never"}
          />
          <Fact label="Rows last built" value={state.lastRowCount.toLocaleString()} />
        </div>

        {state.lastError && (
          <Alert variant="warning" className="text-xs">
            <AlertDescription className="text-inherit">{state.lastError}</AlertDescription>
          </Alert>
        )}
...
function Fact({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="rounded-md border p-2.5">
      ...
      <p className={`text-muted-foreground ${mono ? "font-mono text-2xs" : ""}`}>{value}</p>
    </div>
  );
}

...
- Cursor Desktop may register `cursor://anysphere.cursor-mcp/oauth/callback` and/or `http://localhost:8787/callback`. The private-use URI is stored as a same-origin HTTPS trampoline and handed back to Cursor after authorization. The trampoline accepts only that exact native callback.

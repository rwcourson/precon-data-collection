...
const beforeSha = fs.existsSync(shaFile) ? fs.readFileSync(shaFile, "utf8") : "";

const gen = spawnSync(process.execPath, [path.join(root, "scripts/generate-openapi.mjs")], {
  cwd: root,
  encoding: "utf8",
});
...
if (before && before !== after) {
  process.stderr.write("contract:check failed — openapi.json drifted. Commit regenerated contracts.\n");
  process.exit(1);
}
...
if (!fs.existsSync(openapi)) {
  process.stderr.write("contract:check failed — missing contracts/openapi.json\n");
  process.exit(1);
}
const doc = JSON.parse(after);
const ops = Object.values(doc.paths).flatMap((p) => Object.keys(p)).length;
process.stdout.write(`contract:check passed (${Object.keys(doc.paths).length} paths, ${ops} operations)\n`);

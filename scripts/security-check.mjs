...
const runtimeConfig = fs.readFileSync(path.join(root, "src/lib/runtime-config.ts"), "utf8");
check(
...
check("next.config sets X-Content-Type-Options", nextConfig.includes("X-Content-Type-Options"));
check("next.config sets Content-Security-Policy", nextConfig.includes("Content-Security-Policy"));

const copilotBridge = fs.readFileSync(path.join(root, "src/lib/ai/copilot-bridge.ts"), "utf8");
check(
  "copilot bridge signs timestamp and body hash",
  copilotBridge.includes("input.timestamp") && copilotBridge.includes("sha256Hex(input.rawBody)"),
);
check("copilot bridge enforces a replay skew window", copilotBridge.includes("COPILOT_TOOL_MAX_SKEW_MS"));

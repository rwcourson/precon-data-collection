...
if (process.env.NODE_ENV !== "production") {
  process.stderr.write(
    "The isolated production server requires NODE_ENV=production.\n"
  );
  process.exit(1);
}
if (process.env.APP_ENV !== "demo" || process.env.DATABASE_MODE !== "pglite") {
  process.stderr.write(
    "The isolated server requires explicit demo PGlite configuration.\n"
  );
  process.exit(1);
}
...
if (!isolatedProject || !path.isAbsolute(isolatedProject)) {
  process.stderr.write(
    "The isolated server requires an absolute sanitized project directory.\n"
  );
  process.exit(1);
}
...
if (
  ...
  !path
    .resolve(databaseDir)
    .startsWith(`${path.resolve(isolatedProject)}${path.sep}`)
) {
  process.stderr.write(
    "The isolated server requires a database inside its sanitized project directory.\n"
  );
  process.exit(1);
}
...
server.listen(0, "127.0.0.1", () => {
  ...
  process.stdout.write(
    `ISOLATED_SERVER_READY http://127.0.0.1:${address.port}\n`
  );
});
